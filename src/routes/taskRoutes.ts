...
taskRouter.post("/:id/update", authMiddleware, async (req, res) => {
    try {
        ...
        await TaskService.updateTask(id, userId, { title: title, description: description, dueDate: dueDate, priority: priority, status: status });
...
taskRouter.post("/:id/assign", authMiddleware, async (req, res) => {
    try {
        ...
        res.status(500).json({ message: `Error assigning task: ${e.message}` })
    }
});
...
taskRouter.post("/tag", authMiddleware, async (req, res) => {
    try {
        ...
        res.status(500).json({ message: `Unable to create tag: ${e.message}` });
    }
});
...
taskRouter.post("/:id/tags", authMiddleware, async (req, res) => {
    try {
        ...
        res.status(500).json({ message: `Error adding tag to task: ${e.message}` });
    }
...
taskRouter.delete("/:id/tags/:tagId", authMiddleware, async (req, res) => {
    try {
        ...
        res.status(500).json({ message: `Failed to remove tag from task: ${e.message}` });
    }
...
taskRouter.delete("/tags/:id", authMiddleware, async (req, res) => {
    try {
        ...
        res.status(500).json({ message: `Failed to remove tag: ${e.message}` });
    }
});
...
taskRouter.get("/filter/:status", authMiddleware, async (req, res) => {
    try{
        ...
        res.status(500).json({message: `Error sorting tasks: ${e.message}`});
    }
});
...
taskRouter.get("/sort/:field/:order", authMiddleware, async (req, res) => {
    try {
        ...
        res.status(500).json({message: `Error filtering tasks: ${e.message}`});
    }
});
...
taskRouter.get("/filter/:status/sort/:field/:order", authMiddleware, async (req, res) => {
    try{
        ...
        res.status(500).json({message: `Error filtering and sorting tasks: ${e.message}`});
    }
});
